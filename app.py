...
CASSANDRA_KEYSPACE = "mini_netflix"
...
MONGO_URI = os.getenv("MONGO_URI", "mongo_connect_key (write)")
client = MongoClient(MONGO_URI)
db = client["mini_netflix"]

"""
Cassandra audit sistema
"""

def cassandra_schema(session, path="tables.cql"):
...
def audit_log(user_id, action, details=""):
    try:
        ...
        session.execute("""
            INSERT INTO system_audit_by_time (event_date, event_time, user_id, action, details)
            VALUES (%s, %s, %s, %s, %s)
        """, (event_date, now, str(user_id), action, details))    

        session.execute("""
            INSERT INTO system_audit (user_id, event_time, action, details)
            VALUES (%s, %s, %s, %s)
        """, (str(user_id), now, action, details))
...
def get_audit_logs(user_id):
    ...
    return jsonify({"user_id": user_id, "count": len(logs),"events": logs}), 200


@app.route('/audit/time/<date>', methods=['GET'])
def get_audit_by_date(date):
    rows = session.execute("""
        SELECT event_time, user_id, action, details
        FROM system_audit_by_time
        WHERE event_date=%s
        LIMIT 500
    """, (date,))
    ...
    return jsonify({"date": date, "count": len(data), "events": data}), 200


"""
Chat esybės su cassandra pridėjimas
"""

chat_bp = create_chat_blueprint(session=session, audit_log=audit_log, 
                                jwt_key=JWT_KEY, openai_client=hf_client,
                                openai_model=HF_MODEL)
...
def login():
    ...
    audit_log(user['user_id'], "LOGIN", f"User {user['email']} logged in")
...
def create_review():
    ...
    with redis_client.lock(lock_key, timeout=7):
        ...
        audit_log(user["user_id"], "CREATE_REVIEW", {
            "movie_id": movie_id,
            "rating": rating,
            "comment": comment
            })
...
def update_subscription():
    ...
    audit_log(user["user_id"], "UPDATE_SUBSCRIPTION", {
        "new_plan": plan_name,
        "price": plan_price
        })
...
def add_recently_watched():
    ...
    audit_log(user_id, "ADD_HISTORY", {"movie_id": movie_id, "title": title})
...
def remove_from_history():
    ...
    audit_log(user_id, "REMOVE_HISTORY", {"movie_id": movie_id})
